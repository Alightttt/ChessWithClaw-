...
const regex = /\{\/\* RIGHT DESKTOP COLUMN \*\/\}\s*<div style=\{\{ flex: 1, display: 'flex', flexDirection: 'column', padding: '12px 16px 12px 8px', gap: '8px', overflow: 'hidden', minHeight: 0 \}\}>([\s\S]*?)<\/div>\s*<\/div>\s*\) : \(/;

const match = code.match(regex);
if(match) {
  let desktopRight = match[1];

  // We want to add the action buttons at the top of the right column, then move history (always expanded), then chat.
  // Currently Desktop has Chat then Move History.
...
`;
  ...
  console.log('Could not match right desktop column');
}
