...
const regex = /\{\/\*\s*RIGHT DESKTOP COLUMN\s*\*\/\}\s*<div style=\{\{ flex: 1, display: 'flex', flexDirection: 'column', padding: '12px 16px 12px 8px', gap: '8px', overflow: 'hidden', minHeight: 0 \}\}>([\s\S]*?)<\/div>\s*<\/div>\s*\) : \(/;
const match = code.match(regex);
if(match) {
  let desktopRight = match[1];

  const newDesktopRight = `
            <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
              <button onClick={() => setMoveHistoryOpen(prev => !prev)} style={{ flex: 1, background: '#1c1c1c', border: 'none', borderRadius: '12px', padding: '12px', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#666', cursor: 'pointer' }}>
              ...
              <button style={{ flex: 1, background: '#1c1c1c', border: 'none', borderRadius: '12px', padding: '12px', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#666', position: 'relative', cursor: 'pointer' }}>
...
`;
  ...
  code = code.replace(match[0], `<!-- RIGHT DESKTOP COLUMN -->\n          <div style={{ width: '40%', minWidth: '380px', display: 'flex', flexDirection: 'column', padding: '12px 16px 12px 8px', gap: '8px', overflow: 'hidden', minHeight: 0 }}>${newDesktopRight}</div>\n        </div>\n      ) : (`.replace('<!-- RIGHT DESKTOP COLUMN -->', '{/* RIGHT DESKTOP COLUMN */}'));

  fs.writeFileSync('src/pages/Game.jsx', code);
  console.log('Patch 3 applied');
} else {
  console.log('Match failed on split block');
  
  // Let's try matching the start and end more flexibly
  const startIdx = code.indexOf('{/* RIGHT DESKTOP COLUMN */}');
  const endIdx = code.indexOf(') : (', startIdx);
  if(startIdx !== -1 && endIdx !== -1) {
    // we just replace the whole thing
    // ... wait, if I do that, the old chat section gets lost, but I have the new chat section right here so that's fine.
