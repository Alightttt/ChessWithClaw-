...
const startTag = "{/* RIGHT DESKTOP COLUMN */}";
const endTag = "{/* STEP 4: BOTTOM INFO BAR */}";

let idx1 = code.indexOf(startTag);
// find the last one before ) : ( or similar. Actually wait! In the desktop layout, the Bottom Info bar is also there!
// Let's find BottomStatusBar agentConnected={agentConnected} game={game} agentName={agentName} isMobile={false}
let idx2 = code.indexOf("<BottomStatusBar agentConnected={agentConnected} game={game} agentName={agentName} isMobile={false} />", idx1);

if (idx1 !== -1 && idx2 !== -1) {
    // wait, we just want to replace what's inside the right desktop column wrapper
    let matchStr = code.substring(idx1, idx2);
    
    // The wrapper starts at <div style={{ flex: 1, display: 'flex', flexDirection: 'column'... 
